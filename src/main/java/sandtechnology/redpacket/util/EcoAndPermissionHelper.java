...
public class EcoAndPermissionHelper {
    ...
    public static void setup(RedPacketPlugin plugin) {
        plugin.log(Level.INFO, "初始化经济与权限支持....");
        if (getServer().getPluginManager().getPlugin("Vault") != null) {
            per = getRegisteredProvider(Permission.class);
            if (per != null) {
                plugin.log(Level.INFO, "找到权限插件：" + per.getName());
            } else {
                plugin.log(Level.SEVERE, "未找到支持Vault的权限插件！将使用原版API！");
            }
            eco = getRegisteredProvider(Economy.class);
            if (eco != null) {
                plugin.log(Level.INFO, "找到经济插件：" + eco.getName());
            } else {
                plugin.log(Level.SEVERE, "未找到支持Vault的经济插件！");
            }
        } else {
            plugin.log(Level.SEVERE, "未找到Vault！此插件将被禁用！");
        }
